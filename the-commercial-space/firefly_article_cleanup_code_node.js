// Firefly Aerospace Article Cleanup Code Node
// Extracts and cleans article content from scraped Firefly Aerospace pages
...
for (const item of $input.all()) {
  ...
  // Extract the article content from wysiwyg gutenarea div (inside single__content)
  const contentRegex = /<div[^>]*class="[^"]*wysiwyg[^"]*gutenarea[^"]*"[^>]*>([\s\S]*?)<\/div>/;
  ...
  results.push({
    json: {
      ...
      source: articleMeta.source || 'Firefly Aerospace',

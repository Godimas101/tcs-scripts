// Stoked Space Markdown Generator Code Node
// Combines article metadata and content into markdown format
// Expects: Article Check Filter (metadata) and Article Cleanup (content) nodes

// Get metadata from Article Check Filter node
const metadataItems = $('Article Check Filter').all();
...
if (metadataItems[0]?.json.message) {
  ...
  return [{
    json: {
      markdown_stoked_articles: `# Stoked Space News Articles\n\n*Generated on ${new Date().toLocaleString()}*\n\n${metadataItems[0].json.message}\n`,
      article_count: 0,
...
const contentItems = $('Article Cleanup').all();

// Build markdown content for all articles
let markdown = '# Stoked Space News Articles\n\n';
...
return [{
  json: {
    markdown_stoked_articles: markdown,

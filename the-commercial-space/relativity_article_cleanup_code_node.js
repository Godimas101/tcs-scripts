// Relativity Space Article Cleanup Code Node
// Extracts and cleans article content from scraped Relativity Space pages
...
for (const item of $input.all()) {
  ...
  // Extract the article content from sqs-html-content div
  const contentRegex = /<div[^>]*class="[^"]*sqs-html-content[^"]*"[^>]*data-sqsp-text-block-content[^>]*>([\s\S]*?)<\/div>/;
  ...
  results.push({
    json: {
      ...
      thumbnail: articleMeta.thumbnail || '',  // Relativity articles may not have thumbnails
      source: articleMeta.source || 'Relativity Space',

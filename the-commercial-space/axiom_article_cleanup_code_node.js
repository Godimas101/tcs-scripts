// Axiom Space Article Cleanup Code Node
// Extracts and cleans article content from scraped Axiom Space pages
...
for (const item of $input.all()) {
  ...
  // Extract the article content from post-rich-text w-richtext div
  const contentRegex = /<div[^>]*class="[^"]*post-rich-text[^"]*w-richtext[^"]*"[^>]*>([\s\S]*?)<\/div>/;
  ...
  results.push({
    json: {
      ...
      source: articleMeta.source || 'Axiom Space',

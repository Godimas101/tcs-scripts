...
const metadataItems = $('Article Check Filter2').all();

// Check if we received a "no articles" message
if (metadataItems.length > 0 && metadataItems[0].json.message) {
  // No articles found - return simple markdown
  const noArticlesMessage = metadataItems[0].json.message;
  const markdown = `# Relativity Space News Articles\n\n*Generated on ${new Date().toLocaleString()}*\n\n${noArticlesMessage}\n\n---\n`;
  
  return [{
    json: {
      markdown_relativity_articles: markdown,
      article_count: 0,
...
const contentItems = $('Article Cleanup2').all();

// Build markdown content for all articles
let markdown = '# Relativity Space News Articles\n\n';

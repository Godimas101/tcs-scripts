...
const metadataItems = $('Article Check Filter3').all();
...
if (metadataItems.length > 0 && metadataItems[0].json.message) {
  ...
  const markdown = `# Axiom News Articles\n\n*Generated on ${new Date().toLocaleString()}*\n\n${noArticlesMessage}\n\n---\n`;
  
  return [{
    json: {
      markdown_axiom_articles: markdown,
      article_count: 0,
...
const contentItems = $('Article Cleanup3').all();

// Build markdown content for all articles
let markdown = '# Axiom News Articles\n\n';

// NASA Image Article Code Node - Extracts article content from NASA Image Article HTML
...
for (let item of items) {
  try {
    ...
    // Remove figcaptions (image credits are repeated in text)
    contentHtml = contentHtml.replace(/<figcaption[^>]*>[\s\S]*?<\/figcaption>/g, '');
    
    // Remove the featured image figure wrapper but keep the content
    contentHtml = contentHtml.replace(/<div id="" class="hds-media[^>]*>[\s\S]*?<\/figure>/g, '');
    
    // Remove credit lines at the end (Text credit, Image credit)
    contentHtml = contentHtml.replace(/<p>\s*<em>\s*(Text credit|Image credit):[^<]*<\/em>\s*<\/p>/gi, '');

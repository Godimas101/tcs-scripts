...
for (const item of items) {
  try {
    ...
    const urlMatch = htmlContent.match(/<meta property="og:url" content="([^"]+)"\s*\/?>/);
    ...
    const thumbnailMatch = htmlContent.match(/<meta property="og:image" content="([^"]+)"\s*\/?>/);
    ...
    // Extract main article content from div.entry-content
    const contentStartMatch = htmlContent.match(/<div class="entry-content">/);
    ...
    // Find closing </div><!-- .entry-content -->
    const endMatch = afterContent.search(/<\/div><!-- \.entry-content -->/);
    const allArticleContent = endMatch > 0 ? afterContent.substring(0, endMatch) : afterContent;
    ...
    cleaned = cleaned.replace(/<div[^>]*id=['"]jp-relatedposts['"][^>]*>[\s\S]*?<\/div>/gi, '');
    ...
    // If no images found in content but we have a thumbnail, use that
